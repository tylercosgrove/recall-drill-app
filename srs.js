...
const SRS = {
  ...
  grade(entry, rating, now) {
    const e = Object.assign({}, entry);
    if (rating === true) rating = 'good';
    if (rating === false) rating = 'again';
    ...
    if (rating === 'again') {
      ...
      e.reps += 1;
      if (rating === 'hard') {
        wait = e.interval_min === 0 ? 30 : Math.max(30, Math.floor(e.interval_min * 1.2));
      } else if (rating === 'easy') {
        e.ease = Math.min(3.0, e.ease + 0.15);
        e.step = SRS.LEARNING_STEPS_MIN.length;
        wait = e.interval_min < SRS.GRADUATED_MIN
          ? 4 * SRS.GRADUATED_MIN
          : Math.floor(e.interval_min * e.ease * 1.3);
